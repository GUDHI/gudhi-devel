...
inline std::vector<std::vector<double> > create_Gaussian_filter(std::size_t pixel_radius, double sigma)
{
  ...
  for (int x = -pixel_radius; x <= static_cast<int>(pixel_radius); x++) {
    for (int y = -pixel_radius; y <= static_cast<int>(pixel_radius); y++) {
      ...
      kernel[x + pixel_radius][y + pixel_radius] = (std::exp(-(r * r) / sigma_sqr)) / (3.141592 * sigma_sqr);
...
class Persistence_heat_maps
{
 public:
   ...
   **/
  Persistence_heat_maps() : min_(0), max_(0), f(), erase_below_diagonal(false)
  {
  ...
  Persistence_heat_maps(
      ...
      double min_y = 0,
      double max_y = 1)
      : min_(min_x), max_(max_x), heat_map(number_of_y_pixels), discrete(true)
  {
  ...
  Persistence_heat_maps(
      const std::vector<std::pair<double, double> >& interval,
      const std::function<double(const std::pair<double, double>&, const std::pair<double, double>&)>& kernel)
      : discrete(false), kernel(kernel), interval(interval)
  {
  ...
  bool operator==(const Persistence_heat_maps& rhs) const
  {
    ...
    for (std::size_t i = 0; i != this->heat_map.size(); ++i) {
      for (std::size_t j = 0; j != this->heat_map[i].size(); ++j) {
                                                              ...
                                                              Operation_type&& operation)
  {
    ...
    for (std::size_t i = 0; i != first.heat_map.size(); ++i) {
      std::vector<double> v;
      v.reserve(first.heat_map[i].size());
      for (std::size_t j = 0; j != first.heat_map[i].size(); ++j) {
   ...
   **/
  std::size_t number_of_vectorize_functions() const { return this->number_of_functions_for_vectorization; }
   ...
   **/
  std::size_t number_of_projections_to_R() const { return this->number_of_functions_for_projections_to_reals; }
 ...
 private:
  ...
  std::size_t number_of_functions_for_vectorization;
  std::size_t number_of_functions_for_projections_to_reals;
...
inline void Persistence_heat_maps<Scalling_of_kernels>::construct(
    ...
    double max_)
{
...
#ifdef GUDHI_DEBUG
  std::clog << "min and max passed to construct() procedure: " << min_ << " " << max_ << std::endl;
#endif
  ...
  for (std::size_t pt_nr = 0; pt_nr != intervals_.size(); ++pt_nr) {
  ...
  if (erase_below_diagonal) {
    for (std::size_t i = 0; i != this->heat_map.size(); ++i) {
      for (std::size_t j = i; j != this->heat_map.size(); ++j) {
        this->heat_map[i][j] = 0;
      }
    }
  }
...
template <typename Scalling_of_kernels>
inline std::vector<std::vector<double> > Persistence_heat_maps<Scalling_of_kernels>::check_and_initialize_maps(
    const std::vector<Persistence_heat_maps*>& maps)
{
  // checking if all the heat maps are of the same size:
  GUDHI_CHECK_code(
    for (std::size_t i = 0; i != maps.size(); ++i) {
      GUDHI_CHECK(maps[i]->heat_map.size() == maps[0]->heat_map.size(),
                  "Sizes of Persistence_heat_maps are not compatible.");
      GUDHI_CHECK(maps[i]->heat_map[0].size() == maps[0]->heat_map[0].size(),
                  "Sizes of Persistence_heat_maps are not compatible.");
    }
  );

  const auto& map = maps[0]->heat_map;
                                                                   ...
                                                                   double power) const
{
  if (this->discrete) {
    ...
    if (power < std::numeric_limits<double>::max()) {
      ...
      for (std::size_t i = 0; i != this->heat_map.size(); ++i) {
        for (std::size_t j = 0; j != this->heat_map[i].size(); ++j) {
          auto diff = std::fabs(this->heat_map[i][j] - second.heat_map[i][j]);
