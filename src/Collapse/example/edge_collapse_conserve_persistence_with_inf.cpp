...
std::vector<Persistence_pair> get_persistence_pairs(Simplex_tree& st, int ambient_dim) {
  ...
  std::cout << "initialize_filtration" << std::endl;
  st.initialize_filtration();
  std::cout << "Persistent_cohomology" << std::endl;
  // Compute the persistence diagram of the complex
  Persistent_cohomology pcoh(st);
  // initializes the coefficient field for homology - must be a prime number
  int p = 11;
  std::cout << "init_coefficients" << std::endl;
  pcoh.init_coefficients(p);

  // Default min_interval_length = 0.
  std::cout << "compute_persistent_cohomology" << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  for (Vertex_handle i = 0; i < point_vector.size(); i++) {
    // insert the 2 vertices with a 0. filtration value just like a Rips
    stree_from_collapse.insert_simplex({i}, 0.);
    for (Vertex_handle j = 0; j < i; j++) {
      stree_from_collapse.insert_simplex({i, j}, std::numeric_limits<Filtration_value>::infinity());
    }
  }
  mat_filt_edge_coll.filtered_edge_collapse(
    [&stree_from_collapse](const std::vector<Vertex_handle>& edge, Filtration_value filtration) {
        // insert the edge
        stree_from_collapse.assign_filtration(stree_from_collapse.find(edge), filtration);
      });
