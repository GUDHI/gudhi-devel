...
generate_points_on_moment_curve(std::size_t num_points, int dim, typename Kernel::FT min_x, typename Kernel::FT max_x,
  ...
  for (std::size_t i = 0; i < num_points;) {
    ...
    Point p = k.construct_point_d_object()(dim, coords.begin(), coords.end());
    points.push_back(p);
    ++i;
  }
  return points;
  ...
  if (point_size == 2 * dim) {
    *out++ = k.construct_point_d_object()(point_size, current_point.begin(), current_point.end());
    ...
    for (std::size_t slice_idx = 0; slice_idx < num_slices; ++slice_idx) {
      ...
      generate_grid_points_on_torus_d(k, dim, num_slices, out, radius_noise_percentage, cp2, rng);
    }
  }
}
...
generate_points_on_torus_d(std::size_t num_points, int dim, std::string sample = "random",
  ...
  if (sample == "grid") {
    ...
    std::size_t num_slices = (std::size_t)std::pow(num_points + .5, 1. / dim);
...
generate_points_on_two_spheres_d(std::size_t num_points, int dim, double radius, double distance_between_centers,
  ...
  for (std::size_t i = 0; i < num_points;) {
    ...
    if (radius_noise_percentage > 0.) {
      ...
      typename Kernel::Vector_to_point_d k_vec_to_pt = k.vector_to_point_d_object();
      typename Kernel::Scaled_vector_d k_scaled_vec =  k.scaled_vector_d_object();
      p = k_vec_to_pt(k_scaled_vec(k_pt_to_vec(p), radius_noise_ratio));
    }

    typename Kernel::Translated_point_d k_transl = k.translated_point_d_object();
