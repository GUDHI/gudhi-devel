...
// assumes all cells appearing in boundaries are also contained in the complex
...
// assumes all cells appearing in boundaries are also contained in the complex
// ignoreLastGenerators: max dim if reverse == false, min dim otherwise + if min
template <class MultiFiltrationValue>
                                      ...
                                      bool reverse = false)
{
  ...
  if (numberOfParameters < 0) {
    numberOfParameters = complex.get_number_of_parameters();
  }
  GUDHI_CHECK(numberOfParameters > 0, "Invalid number of parameters!");
  ...
  auto print_fil_values = [&](const Fil& fil) {
    GUDHI_CHECK(fil.num_parameters() == static_cast<unsigned int>(numberOfParameters),
...
{
  auto cpx = build_complex_from_bitmap<typename Slicer::Filtration_value>(vertexValues, shape, numberOfParameters);
  return Slicer(std::move(cpx));
}
