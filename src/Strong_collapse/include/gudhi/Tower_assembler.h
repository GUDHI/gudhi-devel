...
class Tower_assembler
{
  private:
    ...
    Flag_complex_sparse_matrix * flag_Filtration;
    typedef std::vector< std::tuple< double, Vertex, Vertex > > Filtered_sorted_edge_list;

  public:
    Tower_assembler(std::size_t numVert)
    {
        ...
        flag_Filtration = new Flag_complex_sparse_matrix(numVert);
    }

    void build_tower_for_two_cmplxs(Flag_complex_sparse_matrix mat_1, const Flag_complex_sparse_matrix & mat_2,  Map redmap_2,  double filtration_value, std::string outFile) // mat_1 and mat_2 are simplex_trees of K1c and K2c (the collapsed ones), redmap_2 is the map of K2 -> K2c
    {
