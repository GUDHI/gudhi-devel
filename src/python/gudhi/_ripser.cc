...
py::list doit(DistanceMatrix&& dist,
              int max_dimension,
              ...
              unsigned homology_coeff_field)
{
  ...
  std::vector<std::vector<std::array<T, 2>>> dgms;
  {
    ...
    auto output = [&](T birth, T death) {
      ...
      if (birth < death) dgms.back().push_back({birth, death});
    };
  ...
  py::list ret;
  for (auto&& dgm : dgms) ret.append(py::array(py::cast(std::move(dgm))));
  return ret;
}
...
py::list sparse(py::array_t<V> is_,
                py::array_t<V> js_,
                py::array_t<T> fs_,
                int num_vertices,
                int max_dimension,
...
py::list lower_to_coo(py::object low_mat, double max_edge_length)
{
  ...
  return py::make_tuple(
      py::array(py::cast(std::move(is))), py::array(py::cast(std::move(js))), py::array(py::cast(std::move(fs))));
}
...
PYBIND11_MODULE(_ripser, m)
{
  ...
  m.def("_full",
        full<float>,
        py::arg("matrix").noconvert(),
        py::arg("max_dimension") = std::numeric_limits<int>::max(),
        py::arg("max_edge_length") = std::numeric_limits<double>::infinity(),
        py::arg("homology_coeff_field") = 2);
  m.def("_full",
        full<double>,
        py::arg("matrix"),
        py::arg("max_dimension") = std::numeric_limits<int>::max(),
