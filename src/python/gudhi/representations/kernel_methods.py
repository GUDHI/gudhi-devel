...
def _persistence_weighted_gaussian_kernel(
    ...
    if kernel_approx is not None:
        ...
        return (1.0 / (np.sqrt(2 * np.pi) * bandwidth)) * np.matmul(approx1, approx2.T)
    else:
        W = np.matmul(ws1[:, np.newaxis], ws2[np.newaxis, :])
        E = (1.0 / (np.sqrt(2 * np.pi) * bandwidth)) * np.exp(
            -np.square(pairwise_distances(D1, D2)) / (2 * bandwidth * bandwidth)
        )
        return np.sum(np.multiply(W, E))


def _persistence_scale_space_kernel(D1, D2, kernel_approx=None, bandwidth=1.0):
    """
    ...
    return 0.5 * _persistence_weighted_gaussian_kernel(
        DD1, DD2, weight=weight_pss, kernel_approx=kernel_approx, bandwidth=bandwidth
    )


def pairwise_persistence_diagram_kernels(
    X, Y=None, kernel="sliced_wasserstein", n_jobs=None, **kwargs
):
    """
    ...
    if kernel == "sliced_wasserstein":
        ...
        return np.exp(
            -pairwise_persistence_diagram_distances(
                ...
                bandwidth=kwargs["bandwidth_fisher"],
                n_jobs=n_jobs,
            )
            / kwargs["bandwidth"]
        )
        ...
        return _pairwise(
            ...
            XX,
            YY,
            metric=_sklearn_wrapper(_persistence_scale_space_kernel, X, Y, **kwargs),
            n_jobs=n_jobs,
        )
        ...
        return _pairwise(
            ...
            XX,
            YY,
            metric=_sklearn_wrapper(_persistence_weighted_gaussian_kernel, X, Y, **kwargs),
            n_jobs=n_jobs,
        )
    else:
        return _pairwise(
            ...
            XX,
            YY,
            metric=_sklearn_wrapper(kernel, **kwargs),
            n_jobs=n_jobs,
        )
...
class SlicedWassersteinKernel(BaseEstimator, TransformerMixin):
    ...
    def __call__(self, diag1, diag2):
        ...
        """
        return np.exp(
            -_sliced_wasserstein_distance(diag1, diag2, num_directions=self.num_directions)
            / self.bandwidth
        )
...
class PersistenceWeightedGaussianKernel(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        return pairwise_persistence_diagram_kernels(
            X,
            self.diagrams_,
            kernel="persistence_weighted_gaussian",
            bandwidth=self.bandwidth,
            weight=self.weight,
    ...
    def __call__(self, diag1, diag2):
        ...
        """
        return _persistence_weighted_gaussian_kernel(
            diag1,
            diag2,
            weight=self.weight,
            kernel_approx=self.kernel_approx,
...
class PersistenceScaleSpaceKernel(BaseEstimator, TransformerMixin):
    ...
    def __call__(self, diag1, diag2):
        ...
        """
        return _persistence_scale_space_kernel(
            diag1, diag2, bandwidth=self.bandwidth, kernel_approx=self.kernel_approx
        )
...
class PersistenceFisherKernel(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, bandwidth_fisher=1.0, bandwidth=1.0, kernel_approx=None, n_jobs=None):
        """
    ...
    def __call__(self, diag1, diag2):
        ...
        return np.exp(
            -_persistence_fisher_distance(
                diag1, diag2, bandwidth=self.bandwidth_fisher, kernel_approx=self.kernel_approx
            )
            / self.bandwidth
        )
