            ...
            for i in range(len(Xfit)):
                if Xfit[i].shape[0] > 0:
                    for indices, scaler in self.scalers:
                        for I in indices:
                            Xfit[i][:, I] = np.squeeze(
                                scaler.transform(np.reshape(Xfit[i][:, I], [-1, 1]))
                            )
        return Xfit
    ...
    def transform(self, X):
        ...
        if self.use:
            ...
            for diag in X:
                diag_pad = np.pad(
                    diag,
                    ((0, max(0, self.max_pts_ - diag.shape[0])), (0, 1)),
                    "constant",
            ...
            for i in range(num_diag):
                ...
                if diag.shape[1] >= 2:
                    if diag.shape[0] > 0:
                        ...
                        if self.location == "upper":
                            new_diag = thresh_diag[
                                sort_index[: min(self.num_pts, thresh_diag.shape[0])], :
                            ]
                    ...
                    if diag.shape[0] > 0:
                        ...
                        if self.location == "upper":
                            new_diag = thresh_diag[
                                : min(self.num_pts, thresh_diag.shape[0]), :
                            ]
                        if self.location == "lower":
                            new_diag = np.concatenate(
                                [
                                    thresh_diag[min(self.num_pts, thresh_diag.shape[0]) :, :],
                                    diag[~idx_thresh],
                                ],
                                axis=0,
                            )
    ...
    def transform(self, X):
        ...
        if self.use:
            ...
            if self.point_type == "finite":
                Xfit = [
                    diag[diag[:, 1] < self.limit] if diag.shape[0] != 0 else diag for diag in X
                ]
            else:
                Xfit = [
                    diag[diag[:, 1] >= self.limit, 0:1] if diag.shape[0] != 0 else diag
                    for diag in X
                ]
        else:
            Xfit = X
