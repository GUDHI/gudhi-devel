...
def cluster_patch(Binned_data, p_ind, clustering, X, input_type):
    ...
    if len(data_bin) > 1:
        clusters = (
            clustering.fit_predict(X[data_bin, :])
...
class CoverComplexPy(BaseEstimator):
    ...
    def save_to_dot(self, file_name="cover_complex", color_name="color", eps_color=0.1, eps_size=0.1):
        """
        ...
        with open(file_name, "w") as f:
            ...
            for simplex, _ in st.get_skeleton(0):
                cnode = (
                    (1.0 - 2 * eps_color) * (node_info_[simplex[0]]["colors"][0] - minv) / (maxv - minv) + eps_color
                    if maxv != minv
                    else 0
                )
                snode = (
                    (1.0 - 2 * eps_size) * (node_info_[simplex[0]]["size"] - mins) / (maxs - mins) + eps_size
                    if maxs != mins
                    else 1
                )
                f.write(
        ...
        cb = matplotlib.colorbar.ColorbarBase(
            ax, cmap=my_cmap, norm=matplotlib.colors.Normalize(vmin=minv, vmax=maxv), orientation="horizontal"
        )
    ...
    def fit(self, X, y=None, filters=None, colors=None, n_jobs=1, backend="loky"):
        ...
        if self.filter_bnds is None:
            self.filter_bnds = np.hstack(
                [np.min(self.filters, axis=0)[:, np.newaxis], np.max(self.filters, axis=0)[:, np.newaxis]]
            )
        ...
        steps = np.repeat(
            ((self.filter_bnds[:, 1] - self.filter_bnds[:, 0]) / self.resolutions).reshape((num_filters, 1)),
        ...
        comparison = (
            filter_values
            >= np.repeat(left_endpoints.reshape((num_filters, self.resolutions.max(), 1)), num_pts, axis=2)
        ) * (
            filter_values
            < np.repeat(right_endpoints.reshape((num_filters, self.resolutions.max(), 1)), num_pts, axis=2)
        )
        ...
        clusters_list = Parallel(n_jobs=n_jobs, backend=backend)(
            delayed(cluster_patch)(Binned_data, p_ind, self.clustering, X, self.input_type)
            for p_ind in range(len(patches))
        )
...
class GraphInducedComplex(CoverComplexPy):
    ...
    def __init__(
        ...
        *,
        ...
        self.voronoi_samples, self.assignments, self.filter_bnds, self.resolution, self.gain = (
            voronoi_samples,
            assignments,
            filter_bnds,
            resolution,
