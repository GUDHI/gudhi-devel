...
class CoverComplexPy(BaseEstimator):
    ...
    def save_to_dot(self, data_name="data", cover_name="cover", color_name="color", eps_color=.1, eps_size=.1):
        """
        Write the 0-skeleton of the cover complex in a DOT file called "{data_name_cover_name_color_name}.dot", that can be processed with, e.g., neato. The vertices of the cover complex are colored with the first color function, ie, the first column of self.colors.  This function also produces an extra pdf file "colorbar_{color_name}.pdf" containing a colorbar corresponding to the node colors in the DOT file.

        Parameters:
            data_name (string): name to use for the data, default "data"
            cover_name (string): name to use for the cover, default "cover"
            color_name (string): name to use for the color, default "color"
        ...
        f = open(data_name + "_" + cover_name + "_" + color_name + ".dot", "w")
        f.write("graph MAP{")
        cols = []
    ...
    def save_to_txt(self, data_name="data", cover_name="cover", color_name="color"):
        """
        Write the cover complex to a TXT file called "{data_name_cover_name_color_name}.txt", that can be processed with KeplerMapper.
        ...
        """
        st = self.simplex_tree_
        f = open(data_name + "_" + cover_name + "_" + color_name + ".txt", "w")
        f.write(data_name + "\n")
        f.write(cover_name + "\n")
        f.write(color_name + "\n")
        f.write("0 0\n")
...
class GraphInducedComplex(CoverComplexPy):
    ...
    def __init__(self, *, input_type="point cloud", cover="functional", min_points_per_node=0,
        ...
        self.input_type, self.cover, self.mask = input_type, cover, min_points_per_node
