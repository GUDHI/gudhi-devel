...
py::array wrap_persistence_1d(py::array_t<T> data)
{
  ...
  if (buf.ndim != 1) throw std::runtime_error("Data must be a 1-dimensional array");
...
py::list wrap_persistence_2d(py::array_t<double, py::array::c_style | py::array::forcecast> data,
                             double min_persistence)
{
  py::buffer_info buf = data.request();
  if (buf.ndim != 2) throw std::runtime_error("Data must be a 2-dimensional array");
