...
def _LowerStarSimplexTree(
    ...
    simplextree.compute_persistence(
        homology_coeff_field=homology_coeff_field, persistence_dim_max=persistence_dim_max
    )
    ...
    for dimension in dimensions:
        ...
        finite_pairs = (
            pairs[0][dimension] if len(pairs[0]) >= dimension + 1 else np.empty(shape=[0, 2])
        )
        essential_pairs = (
            pairs[1][dimension] if len(pairs[1]) >= dimension + 1 else np.empty(shape=[0, 1])
        )
...
class LowerStarSimplexTreeLayer(tf.keras.layers.Layer):
    ...
    def call(self, filtration):
        ...
        indices = _LowerStarSimplexTree(
            self.simplextree, filtration.numpy(), self.dimensions, self.hcf, self.pdm
        )
