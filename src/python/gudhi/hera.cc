 ...
 *          wasserstein.cc, because nanobind doesn't accept 2 modules named bottleneck_distance
...
nb::object wasserstein_distance(
    const Dgm& d1, const Dgm& d2,
    ...
    double delta, bool return_matching)
{
  ...
  if(dist == std::numeric_limits<double>::infinity())
    return nb::make_tuple(dist, nb::none());
