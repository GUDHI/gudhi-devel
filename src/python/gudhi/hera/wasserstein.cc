...
py::object wasserstein_distance(Dgm d1,
                                Dgm d2,
                                ...
                                bool return_matching)
{
  ...
  if (!return_matching) return py::cast(dist);

  if (dist == std::numeric_limits<double>::infinity()) return py::make_tuple(dist, py::none());
