...
def test_lie_pca(fix_pts):
    ...
    # As group dim is 2, only two eigenvalue should be small, the others should be large
    ...
    assert abs(vals[0]) < 0.015
    assert abs(vals[1]) < 0.015

    for i in vals[2:]:
        assert abs(i) > 0.015

    # Test covariance method for lie_pca
    orbit_fitter = OrbitFitter(fix_pts)
    nb_neighbors = 30
    orbit_dim = 2
