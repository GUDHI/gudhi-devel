...
def test_activities_license_capture():
    for subset in [None, "cross_training", "jumping", "stepper", "walking"]:
        ...
        assert (
            ""
            == _capture_license_output(
                remote.fetch_daily_activities, subset=subset, accept_license=True
            ).getvalue()
        )
...
def test_fetch_remote_datasets_wrapped():
    ...
    for i in range(2):
        ...
        activities_arr = remote.fetch_daily_activities(
            "./another_fetch_folder_for_test/activities.npy", subset=None
        )
        assert activities_arr.shape == (30000, 4)
        assert activities_arr.dtype == np.dtype("float32")

        # Order is important
        walking_arr = remote.fetch_daily_activities(
            "./another_fetch_folder_for_test/activities.npy", subset="walking"
        )
        np.testing.assert_array_equal(
            walking_arr, activities_arr[activities_arr[:, 3] == 9.0][:, 0:3]
        )
        stepper_arr = remote.fetch_daily_activities(
            "./another_fetch_folder_for_test/activities.npy", subset="stepper"
        )
        np.testing.assert_array_equal(
            stepper_arr, activities_arr[activities_arr[:, 3] == 13.0][:, 0:3]
        )
        cross_training_arr = remote.fetch_daily_activities(
            "./another_fetch_folder_for_test/activities.npy", subset="cross_training"
        )
        np.testing.assert_array_equal(
            cross_training_arr, activities_arr[activities_arr[:, 3] == 14.0][:, 0:3]
        )
        jumping_arr = remote.fetch_daily_activities(
            "./another_fetch_folder_for_test/activities.npy", subset="jumping"
        )
        np.testing.assert_array_equal(
            jumping_arr, activities_arr[activities_arr[:, 3] == 18.0][:, 0:3]
        )
