...
def test_flag_generators():
    ...
    assert np.array_equal(
        g[0], [[2, 2, 0], [1, 1, 0], [3, 3, 1], [6, 6, 4], [5, 5, 4], [7, 7, 5]]
    )
    assert len(g[1]) == 1
    ...
    assert {tuple(i) for i in g[0]} == {
        (i[0][0],) + tuple(i[1]) for i in pairs if len(i[0]) == 1 and len(i[1]) != 0
    }
    assert {(i[0], i[1]) for i in g[1][0]} == {
