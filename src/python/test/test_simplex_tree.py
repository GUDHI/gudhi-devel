...
def test_collapse_edges_as_graph():
    st = SimplexTree()
    ...
    st2 = st.collapse_edges_as_graph(inplace=False)
...
def test_collapse_edges_as_flag_complex():
    ...
    st2 = st.collapse_edges_as_flag_complex(inplace=False)
    assert st2.num_simplices() == 11
    assert st2.find([0, 2]) == False  # [1, 3] would be fine as well
    for simplex in st2.get_skeleton(0):
        assert simplex[1] == 1.0

    st3 = st.collapse_edges_as_flag_complex(max_expansion_dim=1, inplace=False)
