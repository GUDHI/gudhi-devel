...
def test_lagrangian_barycenter():
    ...
    assert (
        np.linalg.norm(
            lagrangian_barycenter(pdiagset=[dg1, dg2, dg3, dg4], init=3, verbose=False) - res
        )
        < eps
    )
    assert np.array_equal(
        lagrangian_barycenter(pdiagset=[dg4, dg5, dg6], verbose=False), np.empty(shape=(0, 2))
    )
    ...
    assert (
        np.linalg.norm(
            lagrangian_barycenter(
                pdiagset=[dg8, dg4], init=np.array([[0.2, 0.6], [0.5, 0.7]]), verbose=False
            )
            - np.array([[1, 3], [5, 7]])
        )
        < eps
