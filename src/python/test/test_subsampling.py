    ...
    for fast in [False, True]:
        ...
        for point in point_set:
            ...
            sub_set = gudhi.choose_n_farthest_points(
                points=point_set, nb_points=1, starting_point=i, fast=fast
            )
        ...
        sub_set = gudhi.choose_n_farthest_points(
            points=point_set, nb_points=2, starting_point=1, fast=fast
        )
        assert sub_set[1] == point_set[3]
        sub_set = gudhi.choose_n_farthest_points(
            points=point_set, nb_points=2, starting_point=3, fast=fast
        )
        assert sub_set[1] == point_set[1]
        sub_set = gudhi.choose_n_farthest_points(
            points=point_set, nb_points=2, starting_point=0, fast=fast
        )
        assert sub_set[1] == point_set[2]
        sub_set = gudhi.choose_n_farthest_points(
            points=point_set, nb_points=2, starting_point=2, fast=fast
        )
        assert sub_set[1] == point_set[0]

        # Test the limits
        assert (
            gudhi.choose_n_farthest_points(points=[], nb_points=0, starting_point=0, fast=fast)
            == []
        )
        assert (
            gudhi.choose_n_farthest_points(points=[], nb_points=1, starting_point=0, fast=fast)
            == []
        )
        assert (
            gudhi.choose_n_farthest_points(points=[], nb_points=0, starting_point=1, fast=fast)
            == []
        )
        assert (
            gudhi.choose_n_farthest_points(points=[], nb_points=1, starting_point=1, fast=fast)
            == []
        )

        # From off file test
        for i in range(0, 7):
            r = gudhi.choose_n_farthest_points(
                off_file="subsample.off", nb_points=i, starting_point=i, fast=fast
            )
            assert len(r) == i

    points = np.random.rand(100, 2)
    r1 = gudhi.choose_n_farthest_points(
        points=points, nb_points=10, starting_point=17, fast=False
    )
    r2 = gudhi.choose_n_farthest_points(
        points=points, nb_points=10, starting_point=17, fast=True
    )
    assert r1 == r2
...
def test_simple_choose_n_farthest_points_randomed():
    ...
    for fast in [False, True]:
        ...
        for iter in range(1, 10):
            sub_set = gudhi.choose_n_farthest_points(
                points=point_set, nb_points=iter, fast=fast
            )
            for sub in sub_set:
                found = False
                for point in point_set:
            ...
            assert (
                len(
                    gudhi.choose_n_farthest_points(
                        off_file="subsample.off", nb_points=i, fast=fast
                    )
                )
                == i
            )
