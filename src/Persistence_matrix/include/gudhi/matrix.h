...
//											std::unordered_map<index,Row_type>,
	...
	static const bool dimensionIsNeeded = Options::has_column_pairings && Options::is_of_boundary_type && !Options::has_vine_update && !Options::can_retrieve_representative_cycles;
	...
	Matrix(const std::vector<Boundary_type>& orderedBoundaries,BirthComparatorFunction&& birthComparator, DeathComparatorFunction&& deathComparator);
	template<typename BirthComparatorFunction, typename DeathComparatorFunction>
	Matrix(unsigned int numberOfColumns,BirthComparatorFunction&& birthComparator, DeathComparatorFunction&& deathComparator);
	//*******************
	...
	//pos to id
	template<typename Index_type>
	std::enable_if_t<std::is_integral_v<Index_type> > add_to(Index_type sourceColumnIndex, const Field_type& coefficient, Index_type targetColumnIndex);
	//base
	//base comp
	template<class Cell_range>
	std::enable_if_t<!std::is_integral_v<Cell_range> > add_to(const Cell_range& sourceColumn, const Field_type& coefficient, index targetColumnIndex);
	...
	//pos to id
	template<typename Index_type>
	std::enable_if_t<std::is_integral_v<Index_type> > add_to(const Field_type& coefficient, Index_type sourceColumnIndex, Index_type targetColumnIndex);
	//base
	//base comp
	template<class Cell_range>
	std::enable_if_t<!std::is_integral_v<Cell_range> > add_to(const Field_type& coefficient, const Cell_range& sourceColumn, index targetColumnIndex);
...
inline Matrix<Options>::Matrix(const std::vector<Boundary_type>& orderedBoundaries,BirthComparatorFunction&& birthComparator, DeathComparatorFunction&& deathComparator)
...
template<class Options>
template<typename BirthComparatorFunction, typename DeathComparatorFunction>
inline Matrix<Options>::Matrix(unsigned int numberOfColumns,BirthComparatorFunction&& birthComparator, DeathComparatorFunction&& deathComparator) 
...
template<class Options>
template<class Boundary_type>
inline typename Matrix<Options>::insertion_return_type Matrix<Options>::insert_boundary(index simplexIndex, const Boundary_type &boundary)
{
	assert(Field_type::get_characteristic() != 0 &&
			"Columns cannot be initialized if the coefficient field characteristic is not specified. "
			"Use a compile-time characteristic initialized field type or call coefficient initializer of the chosen field class.");
...
{
	static_assert(isNonBasic && Options::is_of_boundary_type && (Options::has_vine_update || Options::can_retrieve_representative_cycles) && Options::is_indexed_by_position, "Only enabled for position indexed RU matrices.");
...
{
	static_assert(Options::has_removable_columns && !isNonBasic && !Options::has_column_compression, "'erase_column' is not available for the chosen options.");
...
template<class Options>
template<class Cell_range>
inline std::enable_if_t<!std::is_integral_v<Cell_range> > Matrix<Options>::add_to(const Cell_range& sourceColumn, index targetColumnIndex)
{
...
template<class Options>
template<typename Index_type>
inline std::enable_if_t<std::is_integral_v<Index_type> > Matrix<Options>::add_to(Index_type sourceColumnIndex, const Field_type& coefficient, Index_type targetColumnIndex)
{
...
template<class Options>
template<class Cell_range>
inline std::enable_if_t<!std::is_integral_v<Cell_range> > Matrix<Options>::add_to(const Cell_range& sourceColumn, const Field_type& coefficient, index targetColumnIndex)
{
	static_assert(!isNonBasic, "For boundary or chain matrices, only additions with columns inside the matrix is allowed to maintain algebraic consistency.");
...
template<class Options>
template<typename Index_type>
inline std::enable_if_t<std::is_integral_v<Index_type> > Matrix<Options>::add_to(const Field_type& coefficient, Index_type sourceColumnIndex, Index_type targetColumnIndex)
{
...
template<class Options>
template<class Cell_range>
inline std::enable_if_t<!std::is_integral_v<Cell_range> > Matrix<Options>::add_to(const Field_type& coefficient, const Cell_range& sourceColumn, index targetColumnIndex)
{
	static_assert(!isNonBasic, "For boundary or chain matrices, only additions with columns inside the matrix is allowed to maintain algebraic consistency.");
...
{
	static_assert(Options::is_of_boundary_type && !Options::has_column_compression, "'zero_cell' is not available for the chosen options.");
...
{
	static_assert(isNonBasic && Options::is_of_boundary_type && (Options::has_vine_update || Options::can_retrieve_representative_cycles) && Options::is_indexed_by_position, "Only enabled for RU matrices.");
...
{
	static_assert(Options::is_of_boundary_type && !Options::has_column_compression, "'zero_column' is not available for the chosen options.");
...
{
	static_assert(isNonBasic && 
			(!Options::is_of_boundary_type	//chain
			|| (Options::has_vine_update || Options::can_retrieve_representative_cycles)),	//RU
...
{
	static_assert(Options::has_column_pairings, "This method was not enabled.");
	static_assert(!Options::is_of_boundary_type || Options::has_vine_update || Options::can_retrieve_representative_cycles, "'get_current_barcode' is not const for boundary matrices as the barcode is only computed when explicitely asked.");
...
{
	static_assert((!isNonBasic && !Options::has_column_compression) || (isNonBasic && Options::is_of_boundary_type && Options::is_indexed_by_position && !Options::has_vine_update && !Options::can_retrieve_representative_cycles), "This method was not enabled.");
...
{
	static_assert((!isNonBasic && !Options::has_column_compression) || (isNonBasic && Options::is_of_boundary_type && !Options::has_vine_update && !Options::can_retrieve_representative_cycles), "This method was not enabled.");
...
inline constexpr void Matrix<Options>::_assert_options()
{
	...
	//TODO: redo all asserts with help of the unit tests.
