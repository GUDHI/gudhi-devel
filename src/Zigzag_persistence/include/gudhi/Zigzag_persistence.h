...
private: 
	...
	{ //fp is the largest death index for <=d
		...
		auto chain_fp = chains_in_F.front(); //col_fp, with largest death <d index.
		...
		for(auto &chain_f : chains_in_F) { available_birth.insert(births_.at(chain_f)); }
...
public:
	...
	persistence_diagram(Filtration_value shortest_interval = 0., bool include_infinit_bars = false)
	{
		...
		//TODO: better recording?
		if (include_infinit_bars) {
			for (unsigned int i = 0; i < matrix_.get_number_of_columns(); ++i) {
				const auto& col = matrix_.get_column(i);
				if (!col.is_paired())
					diag.emplace_back(col.get_dimension(), birth(col.get_pivot()), std::numeric_limits<Filtration_value>::infinity());
			}
		}
...
private:
	...
	matrix_type                            matrix_; // 0 ... m-1
	std::unordered_map<index,int>									births_;
