...
//the only option for rows is the intrusive list
  ...
  struct matrix_chain {
   ...
   /* Assign a paired chain. */
    ...
    Simplex_key lowest_idx() { return lowest_idx_; }
  ...
  */
  ...
  { //compute index persistence, interval are closed, i.e., [b,d) is stored as 
    ...
    { //insertion of a simplex
      ...
      //Iterator zzit gives all cells (critical or non-critical). We can ignore
      //cells that are not critical for insertions. A Morse pair (t,s) must make 
      // t, s consecutive during insertions.
      //For deletions, they may not be consecutive s, t.
      //This case happens when the deletion of a cell s in a Morse pair
      //(t,s) happens with a different filtration value than the one of the
      //deletion of tau. At the moment of deletion, s is a maximal cell, all keys
      //have been assigned correctly by the filtration_simplex_iterator, both t 
      //and s must be elevated into critical cells. This last operation is
      //implemented in make_pair_critical. s is then removed like a normal cell.
      if(zzit.arrow_direction()) { //forward arrow, only consider critical cells
        if constexpr(Complex::Options::store_morse_matching) {//if Morse:
          //insert only critical cells
          if(cpx_->critical(*zzit)) { forward_arrow(*zzit); }
        }
        else { forward_arrow(*zzit); }
      }
      else { //backward arrow
        if constexpr(Complex::Options::store_morse_matching) {
          if(cpx_->critical(*zzit)) { backward_arrow(*zzit); }//standard deletion
          else { //s removed, t is removed later
            if(zzit->break_morse_pair()) {//mat A becomes matrix A U \{\tau,sigma\}
              make_pair_critical(*zzit); //update the homology matrix
              //make both simplices of the Morse pair critical in the complex
              auto sh_pair = cpx_->paired_with(*zzit);
              cpx_->make_critical(*zzit);     cpx_->make_critical(sh_pair);
            }
          }
        }
...
private:
// // /* Compute the entire zigzag persistent homology of the member complex, and tracks a homology basis for the complex at threshold r, i.e., .*/
// //   void zigzag_persistent_homology(Filtration_value r)
// //   { //compute index persistence, interval are closed, i.e., [b,d) is stored as 


// //   }
//   struct homology_cycle {
//     template<typename CellIterator>
//     homology_cycle( CellIterator beg, CellIterator end
//                   , std::map<Simplex_key, Simplex_handle> &all_simp_
//                   , int dim
//                   , Filtration_value birth)
//     : dim_(dim)
//     , birth_(birth) 
//     , death_(std::numeric_limits<Filtration_value>::infinity())
//     {
//       size_t size=0;
//       for(auto it = beg; it != end; ++it) { ++size; }
//       cycle_.reserve(size);
//       for(auto it = beg; it != end; ++it) {
//         cycle_.push_back( all_simp_[it->key()]; );
//       }
//     }

//     std::vector< Simplex_handle > cycle_;
//     int                           dim_;
//     Filtration_value              birth_;
//     Filtration_value              death_;
//   }
//   struct homology_basis {
//     homology_basis( Complex *cpx, std::list< matrix_chain > &hom_mat
//                   , Filtration_value fil) 
//     : cpx_copy(*cpx)//copy the complex
//     , fil_(fil)
//     {
//       //record the conversion key->Simplex_handle
//       for(auto sh : cpx_copy.complex_simplex_range()) {
//         all_simp_[cpx_copy.key(sh)] = sh;
//       }
//       for(auto &chain : hom_mat) {
//         if(chain.inF()) {//live cycle
//           auto sh = all_simp_[chain.lowest_idx()];//a simplex of the chain
//           basis_.emplace_back(chain.column().begin(), chain.column().end()
//                              , all_simp_
//                              , cpx_copy.dimension(sh);
//                              , chain.birth());
//         }
//       }
//     }
...
//     std::vector< homology_cycle >           basis_;
//   }
//   void homology_basis() {

//   }

//   homology_basis hom_basis_;
...
/* sh is a maximal simplex paired with a simplex tsh in a Morse pair (tau,sigma).
 *
 * sh must be paired with tsh, i.e., tsh == cpx_->paired_with(zzsh); return the
 ...
 */
  ...
  {
    std::cout << "ERROR: function not implemented yet.\n";
    ...
    // // should it be morse_boundary_simplex_range?/boundary_morse_complex(...)
...
public:
  ...
  std::list< interval_index > & index_persistence_diagram() 
