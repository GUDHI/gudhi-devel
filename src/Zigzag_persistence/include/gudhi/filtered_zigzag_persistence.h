...
struct Default_filtered_zigzag_options : Default_zigzag_options {
  ...
  using Hash = std::hash<Cell_key>;         /**< Hash method for Cell_key */
  using KeyEqual = std::equal_to<Cell_key>; /**< Equality comparator for Cell_key */
};
...
class Filtered_zigzag_persistence_with_storage
{
 ...
 private:
   ...
   */
  std::unordered_map<Cell_key, Internal_key, typename Options::Hash, typename Options::KeyEqual> handleToKey_;
