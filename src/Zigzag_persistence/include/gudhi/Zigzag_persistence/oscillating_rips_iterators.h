...
class Identity_edge_modifier
{
 public:
   ...
   */
  template<typename Filtration_value>
  static Filtration_value apply_modifier(Filtration_value f) { return f; }

  /**
   * @brief Returns the given value.
   */
  template<typename Filtration_value>
  static Filtration_value apply_inverse_modifier(Filtration_value f) { return f; }
...
class Oscillating_rips_edge_range
{
 public:
  /**
   * @class Oscillating_rips_edge_iterator oscillating_rips_iterators.h \
   * gudhi/Zigzag_persistence/oscillating_rips_iterators.h
                                                                       ...
                                                                       boost::forward_traversal_tag>
  {
   public:
          ...
          insertVertex_(true)
    {
      ...
      auto it = std::upper_bound(
          distanceMatrix_[1].begin(),
          distanceMatrix_[1].end(),
   ...
   private:
    ...
    {
      auto it = std::upper_bound(
          distanceMatrix_[rowIndex_].begin(),
          distanceMatrix_[rowIndex_].end(),
          std::pair<int, Filtration_value>(distanceMatrix_.size(), mu_ * epsilonValues_[epsilonIndex_]),
          Point_distance_comp());
    ...
    {
      auto it = std::lower_bound(distanceMatrix_[rowIndex_].begin(),
                                 distanceMatrix_[rowIndex_].end(),
  ...
  {
    return boost::iterator_range<Oscillating_rips_edge_iterator>(
        Oscillating_rips_edge_iterator(nu, mu, orderedPoints, distance, epsilonValues),
        Oscillating_rips_edge_iterator());
  }
   ...
   */
  template <typename PointRange, typename DistanceFunction>
  static Oscillating_rips_edge_iterator begin(Filtration_value nu,
                                              Filtration_value mu,
                                              const PointRange& orderedPoints,
                                              DistanceFunction&& distance,
                                              const std::vector<Filtration_value>& epsilonValues)
 ...
 private:
  ...
  /**
   * @brief Initialize the distance function with already given epsilon values. Updates also the multipliers if
   * the edge modifier is active.
   *
   * @tparam PointRange Point range type.
   * @tparam DistanceFunction Type of the distance function.
...
/**
 * @class Oscillating_rips_simplex_range oscillating_rips_iterators.h \
 * gudhi/Zigzag_persistence/oscillating_rips_iterators.h
 ...
 public:
                                      ...
                                      boost::forward_traversal_tag>
  {
   public:
     ...
     * Default value: -1.
     */
    Oscillating_rips_iterator(EdgeRangeIterator& edgeStartIterator,
...
#ifdef GUDHI_USE_TBB
      tbb::parallel_sort(currentSimplices_.begin(), currentSimplices_.end(), reverse_lexicographic_order(complex_));
#else
      std::sort(currentSimplices_.begin(), currentSimplices_.end(), reverse_lexicographic_order(complex_));
#endif
    }
...
#ifdef GUDHI_USE_TBB
      tbb::parallel_sort(
          currentSimplices_.begin(), currentSimplices_.end(), [&](Simplex_handle sh1, Simplex_handle sh2) -> bool {
   ...
   * Default value: -1.
   *
   * @return Instantiation of @ref Oscillating_rips_iterator.
   *
   * @warning Avoid copying the iterator as it is heavier than usual iterators.
   */
  static Oscillating_rips_iterator begin(EdgeRangeIterator& edgeStartIterator,
                                         EdgeRangeIterator& edgeEndIterator,
                                         StableFilteredComplex& complex,
                                         int maxDimension = -1)
  {
   ...
   *
   * @return Default instantiation of @ref Oscillating_rips_iterator.
   */
