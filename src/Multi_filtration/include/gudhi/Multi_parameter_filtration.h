...
class Multi_parameter_filtration
{
 public:
   ...
   */
  template<typename U>
  Multi_parameter_filtration(const Multi_parameter_filtration<U, Co, Ensure1Criticality> &other)
   ...
   */
  template<typename U>
  Multi_parameter_filtration &operator=(const Multi_parameter_filtration<U, Co, Ensure1Criticality> &other)
  {
  ...
  template <class ValueRange,
            class = std::enable_if_t<!std::is_arithmetic_v<ValueRange> &&
  ...
  friend U compute_euclidean_distance_to(const Multi_parameter_filtration &f, const Multi_parameter_filtration &other)
  {
    ...
    if constexpr (Ensure1Criticality) {
      return _compute_frobenius_norm(f.num_parameters(),
                                     [&](size_type p) -> U { return f.generators_[p] - other.generators_[p]; });
    } else {
  ...
  friend U compute_norm(const Multi_parameter_filtration &f)
  {
    ...
    return _compute_frobenius_norm(f.num_entries(), [&](size_type i) -> U { return f.generators_[i]; });
  }
...
class numeric_limits<Gudhi::multi_filtration::Multi_parameter_filtration<T, Co, Ensure1Criticality> >
{
 public:
  ...
  static constexpr Filtration_value quiet_NaN(std::size_t p) noexcept(false)
  {
    if constexpr (std::numeric_limits<T>::has_quiet_NaN) {
